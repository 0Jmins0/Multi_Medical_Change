...
with open(filename, 'r') as file:
    for i, line in enumerate(file):
        ...
        if i == 0:
            ...
            all_time = list(map(float, data_points))

# 确保所有列表长度相同
assert len(numbers) == len(DP_time) == len(force_time) == len(all_time)


# 创建一个元组列表，每个元组包含 (number, DP_time, force_time, all_time)
data = list(zip(numbers, DP_time, force_time, all_time))
...
# 排序后的数据
sorted_numbers, sorted_DP_time, sorted_force_time, sorted_all_time = zip(*sorted_data)
