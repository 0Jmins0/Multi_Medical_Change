...
def read_data_cola(datain):
    ...
    loads.append(0)
    dis_matrix = [[round(d[i] * 10) for i in range(n + 2)] for d in dis]
...
def read_data_random(p):
    ...
    with open(other, newline='') as csvfile:
        ...
        n = int(csv_data[0][0])
        instance['n'] = n
        instance['need'] = list(map(int,csv_data[1]))  # Extract need data
        instance['distance'] = csv_data[2:]  # Extract discharge data
        instance['distance'] = [list(map(int, row)) for row in instance['distance']]
    return instance
